    ...
    async def _generate_scoreboard_image(self, tabulate_list: list, dev_guild: bool = False):
        ...
        for character in tabulate_list[:10]:
            ...
            if (discord_name := await self.fetch_discord_name(char_name)) and dev_guild:
                ...
                discord_name_font = ImageFont.truetype(
                    str(bundled_data_path(self) / "Roboto-Bold.ttf"), 16
                )  # type: ignore
                draw.text((x, y + 22), f"@{discord_name}", class_color, font=discord_name_font)
            else:
