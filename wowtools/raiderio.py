    ...
    async def profile(self, ctx, character: str, *realm: str) -> None:
        ...
        try:
            ...
            async with self.session.request("GET", request_url) as resp:
                ...
                embed.set_author(
                    name=_("Raider.io profile"), icon_url="https://cdnassets.raider.io/images/fb_app_image.jpg"
                )
                embed.set_thumbnail(url=char_image)
                embed.add_field(name=_("__**Mythic+ Score**__"), value=char_score["score"], inline=False)
                ...
                embed.add_field(
                    ...
                    value=_("[Armory]({armory_url}) | [WarcraftLogs]({wcl_url}) | [Raidbots]({raidbots_url})").format(
                        armory_url=armory_url, wcl_url=wcl_url, raidbots_url=raidbots_url
                    ),
                )
    ...
    async def gprofile(self, ctx, *, guild: str = "Jahaci Rumene Kadulje") -> None:
        ...
        try:
            async with self.session.request("GET", request_url) as resp:
                ...
                embed.set_author(name=_("Guild profile"), icon_url=self.bot.user.avatar_url)
