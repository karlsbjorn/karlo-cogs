    ...
    async def profile(self, ctx, character: str, *realm: str) -> None:
        ...
        try:
            ...
            async with self.session.request("GET", request_url) as resp:
                ...
                if (
                    resp.status != 200
                    and profile_data["message"] == "Could not find requested character"
                ):
                ...
                char_raid = profile_data["raid_progression"]["sanctum-of-domination"][
                    "summary"
                ]
    ...
    async def gprofile(self, ctx, *, guild: str = "Jahaci Rumene Kadulje") -> None:
        ...
        try:
            async with self.session.request("GET", request_url) as resp:
                ...
                embed.set_author(
                    name=_("Guild profile"), icon_url=self.bot.user.avatar_url
                )
                embed.add_field(
